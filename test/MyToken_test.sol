...
contract MyTokenTest {
  ...
  function checkTotalSupply() public {
    ...
    Assert.equal(
      token.totalSupply(),
      expectedSupply,
      'Total supply should be 1 million with 18 decimals'
    );
  }
  ...
  function checkDeployerBalance() public {
    ...
    Assert.equal(
      ...
      expectedSupply,
      'Deployer should receive all initial tokens'
    );
  }
  ...
  function checkInvariantBalancesSum() public {
    ...
    Assert.equal(
      sumBefore,
      initialSupply,
      'INV-002: Sum of balances must equal total supply before transfer'
    );
    ...
    Assert.equal(
      sumAfter,
      initialSupply,
      'INV-002: Sum of balances must equal total supply after transfer'
    );
  }
}
