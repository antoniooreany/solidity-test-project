...
contract CounterTest {
  ...
  function checkInitialCountIsZero() public {
    Assert.equal(counter.getCount(), 0, 'FR-001: Initial count should be 0');
  }

  /// @dev FR-002: Increment must increase counter by exactly 1
  function checkIncrementByOne() public {
    counter.increment();
    Assert.equal(counter.getCount(), 1, 'FR-002: Count should be 1 after one increment');
  }
  ...
  function checkMultipleIncrements() public {
    ...
    counter.increment();
    counter.increment();
    Assert.equal(counter.getCount(), 3, 'FR-002: Count should be 3 after three increments');
  }

  /// @dev FR-003: getCount must return the current counter value
  function checkGetCountReturnsValue() public {
    Assert.equal(counter.getCount(), 0, 'FR-003: getCount should return 0 initially');
    counter.increment();
    Assert.equal(counter.getCount(), 1, 'FR-003: getCount should return 1 after increment');
  }
  ...
  function checkCountAfterNIncrements() public {
    ...
    Assert.equal(counter.getCount(), n, 'INV-003: Count should equal number of increments');
  }
  ...
  function checkAnyAddressCanIncrement() public {
    ...
    counter.increment();
    Assert.equal(counter.getCount(), 1, 'FR-005: Any address should be able to increment');
  }
  ...
  function checkCounterNeverDecreases() public {
    ...
    for (uint256 i = 0; i < 3; i++) {
      ...
      Assert.ok(currentCount >= previousCount, 'INV-001: Counter must never decrease');
  ...
  function checkGetCountIsReadOnly() public {
    ...
    Assert.equal(count1, count2, 'INV-002: Calling getCount multiple times must return same value');
  }
}
